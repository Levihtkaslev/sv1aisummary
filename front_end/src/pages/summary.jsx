...
const Staff = () => {
  ...
  const currentRows = encounters.slice(indexOfFirstRow, indexOfLastRow);

  const totalPages = Math.ceil(encounters.length / rowsPerPage);
  ...
  const fetchEncounters = async (range) => {
    ...
    try {
      ...
      const responses = Array.isArray(res.data.data) ? res.data.data : [];
        ...
        const searchuser =
          search === "" ||
          res.doctor.toLowerCase().includes(searchlower) ||
          res.uhid?.toString().includes(searchlower) ||
          res.encounter?.toString().includes(searchlower) ||
          res.patient_name?.toString().includes(searchlower);
        return searchuser;
      });
  ...
  const handleEdit = (data) => {
    navigate("/edit-summary", { state: data });
  };
  ...
  const exportt = () => {
    ...
    const worksheet = XLSX.utils.json_to_sheet(encounters);
  ...
  return (
    <div className='first'>
        ...
        <div className='heading1'>
            <p className='headtitle'>Summary</p>
            <div className='bread'>
                <>
                <><BiSolidHome /></>
      ...
      <div className=" mt-10 w-full  justify-center flex gap-3 items-center">
          <button
